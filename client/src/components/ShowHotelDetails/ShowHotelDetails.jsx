...
const ShowHotelDetails = ({ hotel, avgRating, reviewCount }) => {
  return (
    <div className="max-w-5xl mx-auto my-20 px-4 py-8" data-aos="fade-up">
      <h2 className="text-3xl uppercase font-bold text-center mt-4">{hotel.title}</h2>
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-8">
        ...
        <p className="text-gray-700 text-justify text-lg mb-6">{hotel.description}</p>
        <p className="text-xl font-medium text-gray-800 mb-6">
          For <span className="text-green-600 font-bold">Rs {(hotel.price).toLocaleString("INR")} </span>
        ...
        <p className="text-lg text-gray-600 mb-4 flex items-center gap-x-2">
          <FaMapMarkerAlt className='text-red-500 w-5 h-5 ' /> {hotel.city}, {hotel.state}, {hotel.country}
        </p>
      </div>
    </div>
  );
